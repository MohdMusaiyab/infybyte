  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        const activeVendors = data.stats?.activeVendors || 1;
      ...
      <div className="max-w-7xl mx-auto">
        <div className="mb-8">
          ...
          <p className="text-gray-600">
            ...
            {dashboardData?.vendor?.shopName || "your vendor"}&apos;s
            operations.
          </p>
        </div>
        ...
        <div className="bg-white rounded-2xl p-6 border-2 border-gray-200">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <Link to="/manager/food-courts" className="group flex items-center justify-center gap-2 bg-white text-black px-6 py-4 rounded-xl border-2 border-black hover:bg-black hover:text-white transition-all duration-300 hover:scale-105 font-medium">
              <Store className="w-5 h-5" />
              View Food Court
            </Link>
            <Link to={'/manager/profile'} className="group flex items-center justify-center gap-2 bg-white text-black px-6 py-4 rounded-xl border-2 border-black hover:bg-black hover:text-white transition-all duration-300 hover:scale-105 font-medium">
              ...
              {dashboardData?.foodCourts.map((fc, index) => (
                ...
                >
                  ...
                  >
                    <Building
                      className={`w-5 h-5 ${
                        fc.foodCourt.isOpen ? "text-green-600" : "text-red-600"
                      }`}
                    />
                  </div>
                  <div className="flex-1">
                    <Link to={`/manager/food-courts/${fc.foodCourt.id}`}>
                    <p className="text-sm font-medium text-black">
                      {fc.foodCourt.name}
                    </p>
                    </Link>
                    <p className="text-xs text-gray-500">
                      {fc.foodCourt.location}
                    </p>
                    <p className="text-xs text-gray-500">
                      {fc.itemCount} items available
                    </p>
                  </div>
                  <div
              ...
              {dashboardData?.managers.map((manager, index) => (
                ...
                >
                  ...
                  >
                    <User
                      className={`w-5 h-5 ${
                        manager.isActive ? "text-blue-600" : "text-gray-400"
                      }`}
                    />
                  </div>
                  <div className="flex-1">
