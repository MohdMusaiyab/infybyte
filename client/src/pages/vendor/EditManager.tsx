...
// Matches your actual API JSON structure
interface FoodCourt {
  id?: string;     // from /vendor/foodcourts
  _id?: string;    // from /vendor/managers/:id
  name: string;
  location: string;
  timings?: string;
  isOpen?: boolean;
...
interface Manager {
  ...
  contact_no: string;
  isActive: boolean;
  foodCourts: FoodCourt[]; // This is the array from your JSON
  createdAt: string;
  updatedAt: string;
}
...
const EditManager: React.FC = () => {
  ...
  const fetchManagerData = useCallback(async () => {
    ...
    try {
      ...
      // Extract the first food court's ID if it exists in the array
      const currentFC = managerData.foodCourts && managerData.foodCourts.length > 0 
  ...
  // Logic to find current food court details for display
  const assignedFC = manager?.foodCourts?.[0];

  if (loading) return <div className="p-10 text-center animate-pulse text-gray-500">Loading manager details...</div>;

  return (
    <div className="p-4 lg:p-6">
      <div className="max-w-2xl mx-auto">
        <div className="mb-6">
          <button onClick={() => navigate("/vendor/managers")} className="flex items-center gap-2 text-gray-600 hover:text-black mb-4 group transition-colors">
        ...
        <div className="bg-white rounded-2xl p-6 border-2 border-gray-200 shadow-sm">
          ...
          <div className="bg-gray-50 rounded-xl p-6 border-2 border-gray-200 mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
              <div className="flex flex-col">
                ...
                <span className="font-bold text-black">{manager?.userName}</span>
              </div>
              <div className="flex flex-col">
                <span className="text-gray-500">Email</span>
                <span className="font-bold text-black">{manager?.userEmail}</span>
              </div>
              <div className="flex flex-col">
                <span className="text-gray-500">Assigned Food Court</span>
                <span className="font-bold text-black">
                  {assignedFC ? `${assignedFC.name} (${assignedFC.location})` : "Not Assigned"}
                </span>
              </div>
              <div className="flex flex-col">
                <span className="text-gray-500">Member Since</span>
                <span className="font-bold text-black">
                  {manager ? new Date(manager.createdAt).toLocaleDateString() : "N/A"}
                </span>
              </div>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {/* Food Court Select */}
            <div>
              <label className="block text-sm font-bold text-gray-700 mb-2">Assign Food Court</label>
              <div className="relative">
            ...
            <label className="flex items-center gap-3 p-4 border-2 border-gray-200 rounded-xl cursor-pointer hover:border-black transition-all">
              ...
              <span className="font-medium flex items-center gap-2">
                {formData.isActive ? <UserCheck className="w-5 h-5 text-green-600" /> : <UserX className="w-5 h-5 text-gray-400" />}
                Account Active
              </span>
            </label>

            <div className="flex gap-4 pt-4">
              <button type="button" onClick={() => navigate("/vendor/managers")} className="flex-1 py-3 border-2 border-black rounded-xl font-bold hover:bg-gray-50 transition-all">
                Cancel
              </button>
              <button type="submit" disabled={updating} className="flex-1 py-3 bg-black text-white rounded-xl font-bold hover:bg-gray-800 transition-all disabled:opacity-50">
