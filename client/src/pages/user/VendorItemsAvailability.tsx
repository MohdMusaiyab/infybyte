...
const VendorItemsAvailability: React.FC = () => {
  ...
  const allFoodCourts = Array.from(new Set(
    items.flatMap(item => 
    ...
    .map(item => ({
      ...
      foodCourts: (item.foodCourts || []).filter(fc => {
        const matchesFoodCourt = 
          ...
          fc.foodCourtId === selectedFoodCourt;
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          <span className="text-gray-600">
            {filteredItems.length} item(s) • {filteredItems.reduce((acc, item) => acc + (item.foodCourts?.length || 0), 0)} location(s)
          </span>
          {(searchTerm || selectedCategory !== "all" || selectedFoodCourt !== "all" || selectedTimeSlot !== "all") && (
            <button 
              onClick={() => {
                setSearchTerm("");
        ...
        {filteredItems.length === 0 ? (
          ...
          <div className="space-y-4">
            {filteredItems.map((item) => (
              <div key={item.itemId} className="bg-white rounded-2xl border-2 border-gray-200 overflow-hidden">
                {/* Item Header */}
                <button
                ...
                >
                  <div className="flex items-start gap-4">
                    ...
                    <div className="text-left">
                      <h3 className="font-bold text-black text-lg mb-1">{item.name || 'Unnamed Item'}</h3>
                      <div className="flex items-center gap-3 text-sm text-gray-600">
                        <span className="font-medium">₹{item.basePrice || 0}</span>
                ...
                {expandedItems.has(item.itemId) && item.foodCourts && item.foodCourts.length > 0 && (
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {item.foodCourts.map((fc, index) => (
                          <div
                            ...
                            onClick={() => handleFoodCourtClick(fc.foodCourtId)}
                          ...
                          >
                            <div className="flex items-start justify-between mb-3">
                              <div>
                                <h4 className="font-bold text-black text-sm mb-1">{fc.foodCourtName || 'Unknown Food Court'}</h4>
                                <div className="flex items-center gap-1 text-xs text-gray-600">
                                  <MapPin className="w-3 h-3" />
                                  <span>{fc.location || 'Unknown Location'}</span>
                                </div>
                              </div>
                              <span className={`px-2 py-1 rounded-full text-xs font-medium ${getStatusColor(fc.status)}`}>
                                {fc.status || 'unknown'}
                              </span>
                            </div>
                            ...
                            <div className="flex items-center justify-between text-xs">
                              <div className="flex items-center gap-2">
                                ...
                                <span className="text-gray-600 capitalize">{fc.timeSlot || 'unknown'}</span>
                              </div>
                              <div className="text-right">
                                <div className="font-bold text-black">
                                  ₹{fc.price !== undefined ? fc.price : item.basePrice}
                                </div>
                                {fc.price !== undefined && fc.price !== item.basePrice && (
                                  <div className="text-gray-500 line-through text-xs">₹{item.basePrice}</div>
                                )}
                              </div>
                            </div>
                          </div>
